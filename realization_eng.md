...
Create a data mart for RFM customer segmentation:
    ...
    No updates needed A successful order is an order with status "Closed" Segment users into 5 equal RMF segments, if the number of orders is equal, then order them randomly.
}
...
Develop SQL queries for calculating RFM segments
Create the RFM segmentation data mart
Test the data mart
}
